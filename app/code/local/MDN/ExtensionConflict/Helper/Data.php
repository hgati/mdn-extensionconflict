...
class MDN_ExtensionConflict_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getConfigFilesList(){
		...
		$codePath = Mage::getBaseDir('code'); //NEW CODE BLOCK
			...
			while($namespaceName = readdir($poolDir))
			{
				...
				while($moduleName = readdir($namespaceDir))
				{
					...
					if (file_exists($configXmlPath)){
						// NEW CODE BLOCK START
						$pkg = explode('/',$location.$namespaceName);
						$a = end($pkg);
						if(!array_key_exists($a.'_'.$moduleName, $modulesArray)) continue;
						$b = $modulesArray[$a.'_'.$moduleName];
						if(empty($b)) continue;
						if(!$b->is('active')) continue;
						// NEW CODE BLOCK END
